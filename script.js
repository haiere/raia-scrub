...
function analyzePDF(idx, buffer) {
    ...
    try {
        ...
        var fields = [
            ...
            { key: 'Keywords', fn: function() { return pdfDoc.getKeywords(); }, icon: '🔑', sensitive: false },
            { key: 'Creator', fn: function() { return pdfDoc.getCreator(); }, icon: '💻', sensitive: true },
            { key: 'Producer', fn: function() { return pdfDoc.getProducer(); }, icon: '🏭', sensitive: false },
            { key: 'Creation Date', fn: function() { var d = pdfDoc.getCreationDate(); return d ? (d.toISOString ? d.toISOString() : d) : null; }, icon: '🕐', sensitive: false },
            { key: 'Modification Date', fn: function() { var d = pdfDoc.getModificationDate(); return d ? (d.toISOString ? d.toISOString() : d) : null; }, icon: '✏️', sensitive: false },
        ];
...
function renderReport(idx) {
    ...
    setCardStatus(idx, hasSensitive ? 'warn' : 'info', hasSensitive ? '⚠ ' + meta.filter(function(m) { return m.sensitive; }).length + ' sensitive' : 'Info only');
    reportEl.style.display = 'block';
    var listItems = meta.map(function(m) {
        return '<li class="report-item' + (m.sensitive ? '' : ' safe-item') +
            '" title="' + (m.sensitive ? 'Sensitive data' : 'Info') +
            '"><span class="report-item-icon">' + m.icon + '</span><span class="report-item-label">' + m.key + ':</span><span class="report-item-value">' + escHtml(m.value) + '</span></li>';
    }).join('');
    reportEl.innerHTML =
...
function scrubOne(idx) {
    return new Promise(function(resolve, reject) {
        ...
        scrubPromise.then(function(cleanBlob) {
            ...
            if (file.type.startsWith('image/') && entry.originalSrc) renderBeforeAfter(idx, entry.originalSrc, cleanBlob);
            setFooterMsg(idx, 'Metadata removed. Clean size: ' + formatSize(cleanBlob.size));
            toast(file.name + ' scrubbed successfully!', 'success');
            if (files.length > 1) document.getElementById('downloadZipBtn').style.display = 'inline-flex';
            resolve();
        }).catch(function(err) {
...
function downloadAll() {
    ...
    cleanedBlobs.forEach(function(item) {
        if (item && item.blob) { zip.file(item.filename, item.blob); count++; }
    });
