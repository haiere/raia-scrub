...
The application is built for anyone who wants to share photos or PDFs without exposing personal or location data. It is particularly useful for journalists, photographers, researchers, and privacy-conscious individuals.
...
- Internet connection is required only to load the page and its dependencies from CDN (ExifReader, piexifjs, pdf-lib, and JSZip). After the first load, the application can be cached for offline use.
...
If you wish to serve it from your own web server, simply place the HTML file on your server and ensure the CDN resources are accessible (or download them and adjust the script tags accordingly).
...
Raia Scrub processes all files locally. No data is transmitted over the network. The only external resources loaded are the JavaScript libraries from CDN; these are fetched once and cached. The application does not set tracking cookies, nor does it collect or store any user information.
...
Ensure the file format is supported (JPG, PNG, PDF) and does not exceed 50 MB. If the file is corrupted or encrypted, it may fail to parse.

### Metadata not removed

For some edge cases (e.g., deeply embedded EXIF or XMP data), the scrubbing may not be perfect. The tool uses standard libraries to remove common metadata; however, some proprietary fields might remain.

### PDF scrubbing

The tool uses `pdf-lib` to clear standard document properties. It does not remove all embedded metadata (e.g., XMP streams) in all cases, but it covers the most common fields.
...
- Support for additional file formats (e.g., TIFF, HEIC).
...
- For dependency updates, modify the CDN URLs in the `<script>` tags.
